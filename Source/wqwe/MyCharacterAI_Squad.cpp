...
void AMyCharacterAI_Squad::Tick(float DeltaSeconds) {
	...
	if (bAttack == true) {
		for (INT32 i = 0; i < Squad_Array.Num(); i += 1) {
			...
			// 	Squad_Array[i]->GetBlackboardComponent()->SetValueAsObject(AMyCharacterAI_Controller::BB_EnemyActor, EnemyCharacter);
			// 	Squad_Array[i]->EnemyActor = EnemyCharacter;
...
void AMyCharacterAI_Squad::RecvFindEnemy(AMyCharacterAI_Controller* Sender, AMyCharacter* Enemy) {
	...
	for (INT32 i = 0; i < Other_Squad.Num(); i += 1) {
		...
		//와서 공격할것. 임시로 수색박아놈
		//Other_Squad[i]->Squad_Array[0]->CreateSPM(Enemy->GetActorLocation());
		//Other_Squad[i]->Squad_Array[0]->StartAlert(Enemy->GetActorLocation());
	}
}
