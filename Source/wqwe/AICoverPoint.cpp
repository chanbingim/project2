...
void AAICoverPoint::Tick(float DeltaSeconds) {
	...
	if (IsValid(Player) == true) {
		RefreshCover(Player->GetActorLocation() + FVector(0.0f, 0.0f, 90.0f));
...
void AAICoverPoint::RefreshCover(FVector Location) {
	FBoxCenterAndExtent Ext = FBoxCenterAndExtent(Location, FVector(5000, 5000, 5000));
	...
	for (auto& tCover : Cover_List) {
		...
		if (GetWorld()->LineTraceSingleByChannel(FilterHit, FilterStart, FilterEnd, ECollisionChannel::ECC_GameTraceChannel2) == false) {
			continue;
		}
		...
		if (tCover.Data.bCrouchedCover == true) {
			...
			//DrawDebugLine(GetWorld(), FilterStart, FilterHit.bBlockingHit == true ? FilterHit.Location : FilterEnd, FilterHit.bBlockingHit == true ? FColor::Red : FColor::Green, false, 5, 0, 3);
